            ...
            <v-col cols="12" xs="12" sm="4" md="2" v-for="(card, i) in cards" :key="i">
              <v-item v-slot="{ active, toggle }">
                <v-card :color="active ? 'primary' : 'white'" :class="active ? 'borderme' : 'borderout'"
                  class="d-flex rounded-lg mx-auto" dark height="250" width="400" @click="toggle" flat>
                  <v-row>
                    <v-col cols="12" sm="12">
                      <a :href="card.url" target="_blank">
                        <v-img :src="card.image" class="white--text align-end" minw-width="150px" max-width="170px"></v-img>
